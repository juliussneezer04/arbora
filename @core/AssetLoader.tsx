...
export const AssetLoaderContext = createContext<RefObject<AssetStore> | null>(null);
...
export default function AssetLoader({ urls: urlsProp, placeholder, children }: Props) {
    ...
    useEffect(() => {
        (async () => {
          if (!uniqueUrls.current) return;
            uniqueUrls.current.forEach(async url => {
                try {
    ...
    // useEffect(() => {
        ...
        //     const delay = 2000 + (uniqueUrls.current?.size || 0) * 100;
